...
namespace Eff2EmtGUI
{
    class Eff2EmtConverter
    {
        ...
        protected static Dictionary<int, string> mp3indexFiles = null;

        protected static List<string> SoundBank_Emit;
        protected static List<string> SoundBank_Loop;

        // Return the entry from mp3index.txt on line number abs(ID)
        protected static string mp3indexFile(int ID)
        {
            if (mp3indexFiles == null)
            {
                return ""; // Better not ever get here! But just in case.
            }
